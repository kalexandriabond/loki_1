...
exp_param_directory = parent_directory + '/experimental_parameters/criterion_parameters/'
analysis_directory = parent_directory + '/analysis/'
data_directory = parent_directory + '/data/criterion_data/'

if testing:
    subj_id = 0
    exp_param_file = exp_param_directory + 'test_criterion.csv'
else:
    sub_inf_dlg.show()
    subj_id = int(float(user_input_dict['CoAx ID [####]']))
    session_n = int(float(user_input_dict['Session Number [#]']))
    exp_param_file = exp_param_directory + str(subj_id) + '_' + 'sess' + str(session_n) + '_' + 'criterion'+'.csv'

    if not os.path.exists(exp_param_file):
        sys.exit("Experimental parameter file does not exist.")

if testing:
    output_file_name =  str(0) + '_' + 'sess' + str(0) + '_' + 'criterion' + '_' + str(current_time)
else:
    output_file_name =  str(subj_id) + '_' + 'sess' + str(session_n) + '_' + 'criterion' + '_' + str(current_time)
...
instructions_p3 = ("Do your best to focus on the fixation cross. Press the green button to start the task.")
...
# edited for the button wands. will need to make sure that left/right buttons are uniformly assigned.
# left = yellow, right = blue, for example

inst_key = 'b' #changed this because now there are only two options
