...
%% Get Subject Info %%
prompt={'Subject Initials', 'TR Length (in ms)', 'At Scanner? Yes=1, No=0', ...
    'Order: Order1=1, Order2=0', 'Run Number'};
def={'','2000','1','1',''};
...
DataFileName = strcat('SubjData/',SubjID, '_Run',num2str(runnumber), '_FFALOCPPAEBALocal_',num2str(scanstart(4)),num2str(scanstart(5)),'.txt');
...
%         leftbutton=1;
%         rightbutton=4;
%         key1=31; %corr to r and R index or "Same"
%         key2=32; %corr to s and R middle finger or "Different"
          key1 = KbName('1');
          key2 = KbName('2');
else        
          key1 = KbName('1');
...
%% Wait for Trigger %%  
    if atscanner == 1
        keyCodes(1:256) = 0;
        while keyCodes(51)==0  %51 is keyCode for System76
