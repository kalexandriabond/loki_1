...
directories = list([behavioral_directory, func_directory])
...
try:
    ...
    sys.exit(
        "Format failure. Subject ID should be 2 digits, session number should be 1:11,\
         and run number should be 1:2."
    )
...
reward_t0 = np.round(exp_param.reward_arm1.values, 2).astype("int")
reward_t1 = np.round(exp_param.reward_arm2.values, 2).astype("int")
...
# TODO: display value for each greeble, simple value, not prob.
...
# save data
header = (
    "trial, subj_id, run, LR_choice, value_accuracy, value_LR_solution, reward,"
    + "cumulative_reward, rt, total_trial_time, iti, high_val_cue,"
    +  "id_choice, stim_duration,"
    + "stim_onset, stim_offset, abs_response_time")
